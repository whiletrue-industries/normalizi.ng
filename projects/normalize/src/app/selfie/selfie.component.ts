...
export class SelfieComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public ringTransforms = computed(() => {
    const baseX = -this.faceOffsetX() / this.maskOverlayScale();
    const baseY = this.faceOffsetY() / this.maskOverlayScale();
    return SelfieComponent.RING_INFLUENCES.map(inf => `translate(${baseX * inf}px, ${baseY * inf}px)`);
  });
  public centerRingTransformValue = computed(() => {
    const baseX = -this.faceOffsetX() / this.maskOverlayScale();
    const baseY = this.faceOffsetY() / this.maskOverlayScale();
    ...
    return `translate(${baseX}px, ${baseY}px) scale(${clampedScale})`;
  });
